  ...
  <q-page class="q-pa-md">
    <q-card flat bordered class="q-mb-md">
      <q-card-section class="row items-center">
        <div>
          <div class="text-h6 text-title">Monedas</div>
          <div class="text-caption text-grey-7">Administracion de monedas del sistema</div>
        </div>
        <q-space />
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props" class="text-center">
          <q-btn-dropdown label="Opciones" no-caps size="10px" dense color="primary">
            <q-list>
              <q-item clickable v-close-popup @click="monedaEdit(props.row)">
              ...
              <q-item clickable v-close-popup @click="monedaDelete(props.row.id)">
      ...
      <q-card style="width: 420px; max-width: 95vw">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-subtitle1 text-weight-bold">
            {{ form.id ? 'Editar moneda' : 'Nueva moneda' }}
          </div>
          <q-space />
        ...
        <q-card-section class="q-pt-sm">
          <q-form @submit.prevent="form.id ? monedaPut() : monedaPost()">
            ...
            <q-input v-model="form.simbolo" label="Simbolo" dense outlined class="q-mb-sm" />
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'simbolo', label: 'Simbolo', align: 'left', field: 'simbolo' },
  ...
  methods: {
    ...
    monedaNew () {
      this.form = { codigo: '', nombre: '', simbolo: '', orden: 0, activo: true }
      this.dialog = true
    },
    ...
    monedasGet () {
      this.loading = true
      this.$axios.get('monedas', { params: { solo_activos: 0 } })
        .then(res => { this.rows = res.data.items || [] })
        .catch(e => this.$alert.error(e.response?.data?.message || 'Error cargando monedas'))
        .finally(() => { this.loading = false })
    },

    monedaPost () {
      this.loading = true
      this.$axios.post('monedas', this.form)
        .then(() => {
          this.dialog = false
          this.$alert.success('Moneda creada')
          this.monedasGet()
        })
    ...
    monedaPut () {
      this.loading = true
      this.$axios.put(`monedas/${this.form.id}`, this.form)
        .then(() => {
          this.dialog = false
          this.$alert.success('Moneda actualizada')
          this.monedasGet()
        })
    ...
    monedaDelete (id) {
      this.$alert.dialog('Â¿Desea eliminar la moneda?')
        .onOk(() => {
          this.loading = true
          this.$axios.delete(`monedas/${id}`)
            .then(() => {
              this.$alert.success('Moneda eliminada')
              this.monedasGet()
            })
